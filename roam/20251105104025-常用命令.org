...
* TODO 根据trace分析kafka消费链路，如何提高消费速度
** TODO 确定性能瓶颈点
   - 分析Kafka拉取数据是否慢？（目前64线程并发拉取？速度不应该慢）
   - 检查落库速度是否慢？
   - 检查ACK确认数据是否慢？

** TODO 目前三个主题(create,update,delete)分区都是64个，把fetchetchloop改为多reader
...
** TODO 1.1和1.2马上进行代码调整，然后再跑10分钟压测
   - 根据链路进一步分析
...
** TODO 目标：确定 create 链路哪一段最慢，为后续改造提供依据。
...
** TODO 调整 fetch loop 让 create 优先走 batch（优先级 P1）
...
步骤 1 帮我们精准定位瓶颈。
步骤 2 直接改善吞吐。
步骤 3 作为长期监控，等改造稳定后再配置即可。
请确认先按这个顺序推进，我会先拉 trace 并整理统计。

** TODO 需要分析批量链路，编写相应场景并收集trace指标
...
export ADMIN_TOKEN="$(cat ~/secrets/iam_admin.token)"   # 或改用 IAM_APISERVER_ADMIN_USER/PASS
export CREATE_DATASET='{"usernamePrefix":"k6_create","basePassword":"InitPassw0rd!","emailDomain":"perf.local","phonePrefix":"138","phoneSuffixLength":8,"nicknamePool":["PerfOps","PerfQA","PerfSvc"],"extendTemplates":[{"department":"perf","tags":["baseline"]},{"department":"guard","tags":["parallel"]}],"labels":{"origin":"k6","module":"create"},"extras":{"team":"iam-perf"},"isAdminRatio":0.05,"defaultStatus":1,"sleepBaseline":0.05,"sleepParallel":0.02}'
export BASELINE_DURATION=10m
export INGEST_DURATION=10m
export DUPLICATE_DURATION=10m
export VALIDATION_DURATION=10m
需要管理员令牌可选 ADMIN_USERNAME/ADMIN_PASSWORD。
启动命令：
cd /home/mxl/cdmp-mini/cdmp-mini
IAM_APISERVER_DISABLE_CLIENT_RATE_LIMITER=true \
k6 run --summary-export test/iam-apiserver/user/create/output/k6-summary-$(date +%Y%m%d-%H%M%S).json \
       test/iam-apiserver/user/create/k6/create.js
       若需要验证 delete/update，也可 parallel 跑 test/iam-apiserver/user/delete_force/k6/*.js 和 update/k6/*.js。
压测后收集：
log/perf 下的 trace/raw span，确认 pending_marker_verify、persist_user、batchCreateToDB 的 p95。
Prometheus 指标：consumer_lag, consumer_batch_size, redis_pending_marker_*。
Kafka 消费和提交速率，验证多 reader 和批次逻辑是否生效。
若需要我协助自动化脚本或结果整理，告诉我即可。
